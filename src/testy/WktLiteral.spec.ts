...
export class WktLiteralTestSuite {
    ...
    @Test() // @ts-ignore
    async ErrorPointWktLiteralTest() {
        let obj1 = null
        try{
            WktLiteral.create("Point(2.1 az)");
        }catch (e) {
            obj1 = e
        }
        expect.toBeTrue(obj1 instanceof ErrorWktLiteral)

        let obj2 = null
        try{
            WktLiteral.create("Point(ssdffd 5454)");
        }catch (e) {
            obj2 = e
        }
        expect.toBeTrue(obj2 instanceof ErrorWktLiteral)
        ...
        expect.toBeTrue(obj3 instanceof ErrorWktLiteral)
    }

    @Test() // @ts-ignore
    async ErrorEnvelopeWktLiteralTest() {
        let obj1 = null
        try{
            WktLiteral.create("Envelope( 1.5,1.6 , 1.8 ,       1.9 ))");
        }catch (e) {
            obj1 = e
        }
        expect.toBeTrue(obj1 instanceof ErrorWktLiteral)
